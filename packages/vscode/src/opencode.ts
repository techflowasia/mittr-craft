...
function readMittrCraftSettings(): Record<string, unknown> {
...
function createConfiguredOpencodeBinaryError(raw: string, normalized: string): Error {
  const messageSuffix = 'MittrCraft needs the standalone opencode CLI. Install it and set mittrcraft.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
...
function validateConfiguredOpencodeBinaryForManagedStart(): string | null {
  ...
  try {
    const settings = readMittrCraftSettings();
...
function resolveOpencodeCliPath(): string | null {
  ...
  if (sharedFromMittrCraft && isExecutable(sharedFromMittrCraft) && !isKnownOpenCodeDesktopAppPath(sharedFromMittrCraft)) {
    return sharedFromMittrCraft;
  }
...
async function waitForReady(
  ...
  const outputChannel = vscode.window.createOutputChannel('MittrCraftManager');
