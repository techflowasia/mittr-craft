...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('Mittr Craft');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      ...
      outputChannel?.appendLine(
        `[Mittr Craft] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.openSidebar', async () => {
      ...
      try {
        ...
        outputChannel?.appendLine(`[Mittr Craft] workbench.view.extension.openchamber failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('openchamber.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[Mittr Craft] openchamber.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('Mittr Craft: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[Mittr Craft] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('Mittr Craft: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[Mittr Craft] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('Mittr Craft: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('Mittr Craft: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('Mittr Craft: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('Mittr Craft: Failed to restart API - {0}', String(e)));
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.addToContext', async () => {
      ...
      if (!editor) {
        vscode.window.showWarningMessage(t('Mittr Craft [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Mittr Craft [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('Mittr Craft: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('Mittr Craft: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Mittr Craft [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Mittr Craft [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Mittr Craft [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('Mittr Craft: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`Mittr Craft: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.showOpenCodeStatus', async () => {
      ...
      const lines = [
        ...
        `Mittr Craft version: ${extensionVersion || '(unknown)'}`,
