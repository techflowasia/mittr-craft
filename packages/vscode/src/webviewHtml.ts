...
<head>
  ...
  <title>Mittr Craft</title>
</head>
<body>
  <!-- Initial loading screen with simplified Mittr Craft logo -->
    ...
    if (!devServerUrl) {
      ...
      const tryLoadDevBundle = () => {
        ...
        Promise.resolve()
          ...
          .catch((error) => {
            attempt += 1;
            console.warn('[Mittr Craft] VS Code webview dev bundle unavailable, retrying...', error);
