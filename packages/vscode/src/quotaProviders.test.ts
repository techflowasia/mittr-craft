...
const previousQuotaDataDirectory = process.env.MITTRCRAFT_DATA_DIR;
const temporaryQuotaDataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'mittrcraft-vscode-quota-'));
process.env.MITTRCRAFT_DATA_DIR = temporaryQuotaDataDirectory;
...
after(() => {
  if (previousQuotaDataDirectory === undefined) delete process.env.MITTRCRAFT_DATA_DIR;
  else process.env.MITTRCRAFT_DATA_DIR = previousQuotaDataDirectory;
