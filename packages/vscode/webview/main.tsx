...
const handleLocalApiRequest = async (input: RequestInfo | URL, url: URL, init: RequestInit | undefined, method: string) => {
  ...
  // Dictation runs on the MittrCraft web server (WebSocket + worker); the VS
  ...
  if (pathname.startsWith('/api/openchamber/models-metadata')) {
    try {
      ...
      console.warn('[MittrCraft] Failed to fetch models metadata via bridge, returning empty set:', error);
...
window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (targetUrl && targetUrl.hostname.includes('models.dev')) {
    try {
      ...
      console.warn('[MittrCraft] models.dev request failed via bridge, returning empty metadata:', error);
      return new Response(JSON.stringify({}), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
  }
  ...
  Promise.all([
    ...
    if (currentProviderId && currentModelId) {
      ...
      sessionStore.sendMessage(
        ...
        console.error('[MittrCraft] Failed to send prompt:', error);
      });
    } else {
  ...
  if (!notificationSettingsSyncPromise) {
    notificationSettingsSyncPromise = import('@/lib/persistence')
      ...
      .catch((error) => {
        ...
        console.warn('[MittrCraft] Failed to sync notification settings:', error);
      });
  }
...
import('@openchamber/ui/apps/renderVSCodeApp')
  ...
  .catch((error) => {
    console.error('[MittrCraft] Failed to bootstrap UI:', error);
