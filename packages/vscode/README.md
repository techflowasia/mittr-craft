...
![VS Code Extension](https://github.com/techflowasia/mittr-craft/raw/HEAD/packages/vscode/extension.jpg)

**Like the extension? There's also a [desktop app and web version](https://github.com/techflowasia/mittr-craft) with even more features.**
...
Plus everything from the shared MittrCraft UI: branchable timeline, smart tool UIs, voice mode, Git workflows, and more.

## Commands

| Command | Description |
|---------|-------------|
| `MittrCraft: Focus Chat` | Focus the chat panel |
| `MittrCraft: New Session` | Start a new chat session |
| `MittrCraft: Open Sidebar` | Open the MittrCraft sidebar |
| `MittrCraft: Open Agent Manager` | Launch parallel multi-model runs |
| `MittrCraft: Open Session in Editor` | Open current or new session in an editor tab |
| `MittrCraft: Settings` | Open extension settings |
| `MittrCraft: Restart API Connection` | Restart the OpenCode API process |
| `MittrCraft: Show OpenCode Status` | Debug info for development or bug reports |

### Right-click menu

Select code in the editor, right-click, and find the **MittrCraft** submenu:
...
|---------|---------|-------------|
| `mittrcraft.apiUrl` | _(empty)_ | URL of an external OpenCode API server. Leave empty to auto-start a local instance. |
| `mittrcraft.opencodeBinary` | _(empty)_ | Absolute path to the `opencode` CLI binary. Useful when PATH lookup fails. Requires window reload to apply. |
...
- `MITTRCRAFT_VSCODE_BIN=cursor bun run vscode:dev`
- `MITTRCRAFT_VSCODE_DEV_WORKSPACE=/path/to/workspace bun run vscode:dev`
...
Install locally: `code --install-extension packages/vscode/mittrcraft-*.vsix`
