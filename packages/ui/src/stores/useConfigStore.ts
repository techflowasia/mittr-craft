...
interface ConfigStore {
    ...
    // MittrCraft settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and MittrCraft settings in parallel. OpenCode config
                            ...
                            // Detect invalid MittrCraft settings so we can clear them from storage.
