...
import { getWorktreeSetupWaitEnabled } from "@/lib/mittrcraftConfig"
...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsMittrCraftCreated: (sessionId: string) => void
  isMittrCraftCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewMittrCraftSession: (sessionId: string, agents: unknown[]) => void
...
export async function materializeOpenDraftSession(selection: {
  ...
  store.initializeNewMittrCraftSession(created.id, configState.agents ?? [])
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsMittrCraftCreated: (sessionId) =>
    set((s) => {
  ...
  isMittrCraftCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  initializeNewMittrCraftSession: () => {
