...
// foreign platform's chrome, not MittrCraft's own status tokens, so the
...
export const WindowsWindowControls = React.memo(function WindowsWindowControls({
  ...
  useEffect(() => {
    ...
    window.addEventListener('mittrcraft:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('mittrcraft:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
