...
export const WorktreeSectionContent: React.FC<WorktreeSectionContentProps> = ({ projectRef: projectRefProp = null, sections = 'all' }) => {
  ...
  const persistSetupCommands = React.useCallback(async (commands: string[]): Promise<boolean> => {
    ...
    try {
      ...
      if (!ok) {
        toast.error(t('settings.mittrcraft.worktrees.setup.toast.saveFailed'));
        return false;
      }
  ...
  const setupTooltip = (
    <SettingsInfoHint>
      {t('settings.mittrcraft.worktrees.setup.tooltipPrefix')}
      {' '}
      <code className="font-mono text-xs bg-sidebar-accent/50 px-1 rounded">$ROOT_PROJECT_PATH</code>
      {' '}
      {t('settings.mittrcraft.worktrees.setup.tooltipSuffix')}
    </SettingsInfoHint>
  );

  const listTooltip = (
    <SettingsInfoHint>
      {t('settings.mittrcraft.worktrees.list.tooltip')}
    </SettingsInfoHint>
  );
    ...
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.mittrcraft.worktrees.state.selectProject')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  if (isGitRepoLocal === false) {
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.mittrcraft.worktrees.state.gitOnly')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  return (
    <>
      ...
      >
        {isLoadingCommands ? (
          <p className="typography-meta text-muted-foreground">{t('settings.mittrcraft.worktrees.setup.loading')}</p>
        ) : (
          <div className={cn('space-y-2', PROJECT_SETTINGS_CONTROL_WIDTH)}>
            {setupCommands.map((command, index) => (
              <div key={index} className="flex w-full gap-2">
                <Input
                  ...
                  placeholder={t('settings.mittrcraft.worktrees.setup.commandPlaceholder')}
                ...
                <Button
                  ...
                  aria-label={t('settings.mittrcraft.worktrees.setup.removeCommandAria')}
                >
            ...
            >
              ...
              {t('settings.mittrcraft.worktrees.setup.addCommand')}
            </Button>
            <label
            ...
            >
              <Checkbox
                ...
                ariaLabel={t('settings.mittrcraft.worktrees.setup.waitForCommandsAria')}
              />
              <span className={cn(
      ...
      <ProjectSettingsSubsection
        title={t('settings.mittrcraft.worktrees.list.title')}
