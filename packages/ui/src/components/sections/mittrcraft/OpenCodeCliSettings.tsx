...
export const OpenCodeCliSettings: React.FC = () => {
  ...
  return (
    <SettingsSection title={t('settings.mittrcraft.opencodeCli.title')}>
      <div className="space-y-0.5">
        <SettingsFieldRow
          settingsItem="sessions.opencode-binary"
          label={t('settings.mittrcraft.opencodeCli.field.binaryPath')}
          info={(
            <>
              {t('settings.mittrcraft.opencodeCli.tipPrefix')}
              {' '}
              <span className="font-mono">OPENCODE_BINARY</span>
              {' '}
              {t('settings.mittrcraft.opencodeCli.tipMiddle')}
              {' '}
              <span className="font-mono">~/.config/mittrcraft/settings.json</span>
              {'.'}
            </>
          )}
          alignEnd={false}
        ...
        >
          <Input
            ...
            placeholder={t('settings.mittrcraft.opencodeCli.field.binaryPathPlaceholder')}
          ...
          <Button
            ...
            aria-label={t('settings.mittrcraft.opencodeCli.actions.browseAria')}
            title={t('settings.mittrcraft.opencodeCli.actions.browse')}
          >
        ...
        <SettingsInset className={SETTINGS_OPTION_STACK_CLASS}>
          {!isWindowsArm64() && (
            <SettingsCheckboxRow
              ...
              label={t('settings.mittrcraft.opencodeCli.field.showUpdateNotifications')}
              ariaLabel={t('settings.mittrcraft.opencodeCli.field.showUpdateNotificationsAria')}
            />
          )}
