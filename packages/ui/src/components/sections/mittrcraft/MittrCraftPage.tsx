...
import { MittrCraftVisualSettings } from './MittrCraftVisualSettings';
...
import type { MittrCraftSection } from './types';
...
interface MittrCraftPageProps {
    /** Which section to display. If undefined, shows all sections (mobile/legacy behavior) */
    section?: MittrCraftSection;
}

export const MittrCraftPage: React.FC<MittrCraftPageProps> = ({ section }) => {
    const { t } = useI18n();
    ...
    if (!section) {
        return (
            <SettingsPageLayout showSaveStatus className="mittrcraft-page-body space-y-3 sm:space-y-6">
                <MittrCraftVisualSettings />
                <DefaultsSettings />
                {showDesktopNetworkSettings && <DesktopNetworkSettings />}
                {!isVSCode && <OpenCodeCliSettings />}
                {!isVSCode && <MittrCraftToolsSettings />}
    ...
    return (
        <SettingsPageLayout
            ...
            showSaveStatus
            className="mittrcraft-page-body"
        >
...
const GeneralSectionContent: React.FC = () => {
    ...
    return (
        <>
            ...
            {!isVSCode && <OpenCodeCliSettings />}
            {!isVSCode && <MittrCraftToolsSettings />}
            <MittrCraftVisualSettings visibleSettings={[
                'fileEditorKeymap',
                'autoSaveEnabled',
