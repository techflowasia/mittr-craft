...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __MITTRCRAFT_VSCODE_THEME__?: Theme }).__MITTRCRAFT_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('mittrcraft:theme-hmr', handleThemeHmr);
    return () => window.removeEventListener('mittrcraft:theme-hmr', handleThemeHmr);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('mittrcraft:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('mittrcraft:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __mittrcraftApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__mittrcraftApplyThemeSync = applyIncomingThemeSync;

    if (receivesParentThemeSync && window.parent !== window) {
      window.parent.postMessage({ type: 'mittrcraft:theme-sync-request' }, window.location.origin);
    }

    return () => {
      if (scopedWindow.__mittrcraftApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__mittrcraftApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'mittrcraft:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('mittrcraft:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('mittrcraft:settings-synced', handleSettingsSynced);
  }, [receivesParentThemeSync]);
