...
afterAll(() => {
  ...
  if (createdWindow) {
    ...
    delete getWindow().__MITTRCRAFT_HOME__;
  }
...
describe('applyPersistedHomeDirectoryToWindow', () => {
  ...
  test('does not overwrite an injected desktop home directory', () => {
    getWindow().__MITTRCRAFT_HOME__ = '/Users/example';

    applyPersistedHomeDirectoryToWindow('/Users/example/projects/app');

    expect(getWindow().__MITTRCRAFT_HOME__).toBe('/Users/example');
  });

  test('uses persisted home when no runtime home was injected', () => {
    applyPersistedHomeDirectoryToWindow('/Users/example/projects/app');

    expect(getWindow().__MITTRCRAFT_HOME__).toBe('/Users/example/projects/app');
  });
});
