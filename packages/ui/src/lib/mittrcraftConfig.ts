 ...
 * Stores per-project settings in ~/.config/mittrcraft/<projectId>.json.
 * Migrates from legacy <project>/.mittrcraft/mittrcraft.json.
 */
...
const CONFIG_FILENAME = 'mittrcraft.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.mittrcraft';
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'mittrcraft', 'projects'];
...
interface MittrCraftConfig {
  projectPath?: string;
  'setup-worktree'?: string[];
  'setup-worktree-wait'?: boolean;
  projectNotes?: string;
  projectTodos?: MittrCraftProjectTodoItem[];
  projectPlanFiles?: MittrCraftProjectPlanFileLink[];
  projectActions?: MittrCraftProjectAction[];
  projectActionsPrimaryId?: string;
  draftStarters?: DraftStarterRef[];
}

type MittrCraftProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface MittrCraftProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: MittrCraftProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface MittrCraftProjectActionsState {
  actions: MittrCraftProjectAction[];
  primaryActionId: string | null;
}

export interface MittrCraftProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface MittrCraftProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface MittrCraftProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface MittrCraftProjectNotesTodos {
  notes: string;
  todos: MittrCraftProjectTodoItem[];
}

export interface MittrCraftProjectContextData extends MittrCraftProjectNotesTodos {
  plans: MittrCraftProjectPlanFileLink[];
}

export const MITTRCRAFT_PROJECT_NOTES_MAX_LENGTH = 3000;
export const MITTRCRAFT_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
const MITTRCRAFT_PROJECT_ACTION_NAME_MAX_LENGTH = 80;
const MITTRCRAFT_PROJECT_ACTION_COMMAND_MAX_LENGTH = 4000;
const MITTRCRAFT_PROJECT_ACTION_OPEN_URL_MAX_LENGTH = 2000;
const MITTRCRAFT_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH = 300;
const MITTRCRAFT_PROJECT_PLAN_TITLE_MAX_LENGTH = 160;

const MITTRCRAFT_ACTION_PLATFORM_SET = new Set<MittrCraftProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // In some runtimes, window.__MITTRCRAFT_HOME__ can be workspace/project-root
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, MITTRCRAFT_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): MittrCraftProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: MittrCraftProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), MITTRCRAFT_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectPlanFileLinks = (value: unknown): MittrCraftProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: MittrCraftProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): MittrCraftProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: MittrCraftProjectActionPlatform[] = [];
  const seen = new Set<MittrCraftProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as MittrCraftProjectActionPlatform;
    if (!MITTRCRAFT_ACTION_PLATFORM_SET.has(normalized) || seen.has(normalized)) {
      continue;
    }
    seen.add(normalized);
...
const sanitizeProjectActions = (value: unknown): MittrCraftProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: MittrCraftProjectAction[] = [];
  const seenIds = new Set<string>();
  ...
  for (const entry of value) {
    ...
    const id = typeof record.id === 'string' ? record.id.trim() : '';
    const name = trimToMaxLength(typeof record.name === 'string' ? record.name.trim() : '', MITTRCRAFT_PROJECT_ACTION_NAME_MAX_LENGTH);
    const command = trimToMaxLength(typeof record.command === 'string' ? record.command.trim() : '', MITTRCRAFT_PROJECT_ACTION_COMMAND_MAX_LENGTH);
    ...
    const openUrl = trimToMaxLength(openUrlRaw, MITTRCRAFT_PROJECT_ACTION_OPEN_URL_MAX_LENGTH);
    ...
    const desktopOpenSshForward = trimToMaxLength(
      ...
      MITTRCRAFT_PROJECT_ACTION_DESKTOP_FORWARD_MAX_LENGTH
    );
...
} | null | undefined): MittrCraftProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): MittrCraftProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): MittrCraftProjectContextData => {
...
const sanitizePlanTitle = (value: string): string => {
  return trimToMaxLength(value.trim(), MITTRCRAFT_PROJECT_PLAN_TITLE_MAX_LENGTH);
};
 ...
 */
async function readMittrCraftConfig(project: ProjectRef): Promise<MittrCraftConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): MittrCraftConfig | null => {
    ...
    try {
      ...
      return parsed as MittrCraftConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.mittrcraft/mittrcraft.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local mittrcraft.json -> ~/.config/mittrcraft/projects/<projectId>.json
  ...
  try {
    const wrote = await writeMittrCraftConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyMittrCraftConfig(projectDirectory);
    }
  } catch {
 ...
 */
async function writeMittrCraftConfig(
  project: ProjectRef,
  config: MittrCraftConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return false;
  ...
  try {
    ...
    console.error('Failed to write mittrcraft config:', error);
    return false;
  }
}
 ...
 */
async function updateMittrCraftConfig(
  project: ProjectRef,
  updates: Partial<MittrCraftConfig>
): Promise<boolean> {
  const existing = await readMittrCraftConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeMittrCraftConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readMittrCraftConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateMittrCraftConfig(project, { 'setup-worktree': filtered });
}
...
export async function saveWorktreeSetupWaitEnabled(project: ProjectRef, enabled: boolean): Promise<boolean> {
  return updateMittrCraftConfig(project, { 'setup-worktree-wait': enabled });
}
...
export async function saveProjectDraftStarters(project: ProjectRef, starters: DraftStarterRef[]): Promise<boolean> {
  return updateMittrCraftConfig(project, { draftStarters: sanitizeStarterRefs(starters) });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<MittrCraftProjectNotesTodos> {
  const config = await readMittrCraftConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: MittrCraftProjectNotesTodos
): Promise<boolean> {
  ...
  return updateMittrCraftConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<MittrCraftProjectContextData> {
  const config = await readMittrCraftConfig(project);
...
async function getProjectPlanFiles(project: ProjectRef): Promise<MittrCraftProjectPlanFileLink[]> {
  const config = await readMittrCraftConfig(project);
...
async function saveProjectPlanFiles(
  project: ProjectRef,
  value: MittrCraftProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<MittrCraftProjectPlanFile | null> {
...
): Promise<MittrCraftProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<MittrCraftProjectActionsState> {
  const config = await readMittrCraftConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: MittrCraftProjectActionsState
): Promise<boolean> {
...
async function deleteLegacyMittrCraftConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
