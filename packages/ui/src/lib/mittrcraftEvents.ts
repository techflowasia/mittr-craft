...
type MittrCraftEvent = ScheduledTaskRanEvent | SessionCreatedEvent | BrowserControlRequestEvent;
type Listener = (event: MittrCraftEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'mittrcraft:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'mittrcraft:heartbeat') {
    return;
  }

  if (envelope.type === 'mittrcraft:session-created') {
  ...
  if (envelope.type === 'mittrcraft:browser-control-request') {
    const properties = getEventProperties(envelope.properties);
  ...
  if (envelope.type !== 'mittrcraft:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const canControlBrowser = typeof window !== 'undefined' && Boolean(window.__MITTRCRAFT_ELECTRON__);
...
export const subscribeMittrCraftEvents = (listener: Listener): (() => void) => {
