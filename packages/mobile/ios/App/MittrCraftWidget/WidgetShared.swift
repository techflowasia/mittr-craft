...
enum WidgetStore {
    static let appGroup = "group.com.mittrcraft.app"
...
enum WidgetDeepLink {
    static func newSession() -> URL { URL(string: "mittrcraft://new")! }
    static func attention() -> URL { URL(string: "mittrcraft://sessions?filter=attention")! }
    static func status() -> URL { URL(string: "mittrcraft://status")! }
    static func settings() -> URL { URL(string: "mittrcraft://settings")! }
    static func changes() -> URL { URL(string: "mittrcraft://changes")! }
    static func files() -> URL { URL(string: "mittrcraft://view/files")! }
    static func instances() -> URL { URL(string: "mittrcraft://view/instances")! }
    static func session(_ id: String) -> URL {
        let encoded = id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? id
        return URL(string: "mittrcraft://session/\(encoded)") ?? newSession()
    }
}
...
// MARK: - Logo (full MittrCraft mark drawn from the SVG)

/// The MittrCraft logo, drawn to match packages/web/public/logo-dark-512x512.svg: an
