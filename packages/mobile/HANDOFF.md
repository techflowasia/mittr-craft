...
- App id / package: `com.mittrcraft.app`; app name `MittrCraft`.
...
- **Deep links** — `mittrcraft://` URL scheme; a reusable intent vocabulary (`apps/deepLinks.ts`)
...
- **iOS widgets + Control Center + Notification Service Extension** — WidgetKit extension
  (`MittrCraftWidget`), a Control Center control, and an NSE (`MittrCraftNotificationService`)
  that refreshes widgets from push. All share the App Group `group.com.mittrcraft.app`.
...
- Extensions: `MittrCraftWidget` (WidgetKit, deployment 17.0) and `MittrCraftNotificationService`
  (NSE, 15.5), both hand-wired into `App.xcodeproj/project.pbxproj` and embedded via a copy phase.
- App Group `group.com.mittrcraft.app` in all three targets' entitlements (app + widget + NSE).
- `Info.plist`: `CFBundleURLTypes` scheme `mittrcraft`, `NSCameraUsageDescription`.
- Push entitlement (aps-environment) required.
- APNs `mutable-content: 1` (set server/relay side) wakes the NSE to refresh widgets.

### Android (`android/app`)

- `google-services.json` (committed; Firebase project `mittrcraft-8bf7e`). The Google Services
...
- Apple Developer account; App IDs for the app **and** both extensions
  (`com.mittrcraft.app`, `.MittrCraftWidget`, `.MittrCraftNotificationService`), each enabled for
