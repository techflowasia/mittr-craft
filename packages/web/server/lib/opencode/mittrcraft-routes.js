...
function resolveSystemdServiceUnit(environment) {
  ...
  const configuredUnit = typeof environment.MITTRCRAFT_SYSTEMD_UNIT === 'string'
    ? environment.MITTRCRAFT_SYSTEMD_UNIT.trim()
    : '';
  const unit = configuredUnit || 'mittrcraft.service';
...
export const registerMittrCraftRoutes = (app, dependencies) => {
  const {
    fs,
    path,
    process,
  ...
  app.get('/api/mittrcraft/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/mittrcraft/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(mittrcraftDataDir, 'run', `mittrcraft-${currentPort}.json`);
      ...
      if (isForegroundService) {
        ...
        const updateJobName = `mittrcraft-update-${Date.now()}`;
      ...
      let restartCmdFallback = `mittrcraft serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(mittrcraftDataDir, 'update-install.log');
      const logPreamble = [
        '',
  ...
  app.get('/api/mittrcraft/models-metadata', async (_req, res) => {
    try {
