...
const originalBundledOpencodeCliDir = process.env.MITTRCRAFT_BUNDLED_OPENCODE_CLI_DIR;
const originalResourcesPath = process.resourcesPath;
const originalWslBinary = process.env.WSL_BINARY;
const originalMittrCraftWslBinary = process.env.MITTRCRAFT_WSL_BINARY;
...
afterEach(() => {
  ...
  if (typeof originalBundledOpencodeCliDir === 'string') {
    process.env.MITTRCRAFT_BUNDLED_OPENCODE_CLI_DIR = originalBundledOpencodeCliDir;
  } else {
    delete process.env.MITTRCRAFT_BUNDLED_OPENCODE_CLI_DIR;
  }
  ...
  if (typeof originalMittrCraftWslBinary === 'string') {
    process.env.MITTRCRAFT_WSL_BINARY = originalMittrCraftWslBinary;
  } else {
    delete process.env.MITTRCRAFT_WSL_BINARY;
  }
});
...
describe('OpenCode env runtime', () => {
  it('searches an explicit PATH without mutating the process environment', () => {
    const defaultDir = createTempDir('mittrcraft-default-path-');
    const explicitDir = createTempDir('mittrcraft-explicit-path-');
  ...
  it('throws a specific error for a configured directory without an executable CLI in strict mode', async () => {
    const dir = createTempDir('mittrcraft-opencode-dir-');
  ...
  it('applies a valid configured executable OpenCode binary', async () => {
    const dir = createTempDir('mittrcraft-opencode-bin-');
  ...
  it('prefers the bundled CLI over a user-installed OpenCode from PATH', () => {
    const bundledDir = createTempDir('mittrcraft-bundled-opencode-');
    const bundledBinary = path.join(bundledDir, process.platform === 'win32' ? 'opencode.exe' : 'opencode');
    const pathDir = createTempDir('mittrcraft-path-opencode-');
    ...
    process.env.MITTRCRAFT_BUNDLED_OPENCODE_CLI_DIR = bundledDir;
  ...
  it('keeps explicit OpenCode binary ahead of bundled CLI', () => {
    const bundledDir = createTempDir('mittrcraft-bundled-opencode-');
    const bundledBinary = path.join(bundledDir, process.platform === 'win32' ? 'opencode.exe' : 'opencode');
    const explicitDir = createTempDir('mittrcraft-explicit-opencode-');
  ...
  it('resolves the bundled OpenCode CLI from Electron resourcesPath', () => {
    const resourcesPath = createTempDir('mittrcraft-resources-');
    ...
    process.env.PATH = createTempDir('mittrcraft-empty-path-');
    delete process.env.MITTRCRAFT_BUNDLED_OPENCODE_CLI_DIR;
    delete process.env.OPENCODE_BINARY;
    const emptyHome = createTempDir('mittrcraft-empty-home-');
    const { runtime, state } = createRuntime({}, {
  ...
  it('rejects known Windows OpenCode desktop app install paths', async () => {
    setPlatform('win32');
    const localAppData = createTempDir('mittrcraft-localappdata-');
  ...
  it('does not auto-detect the Windows OpenCode desktop app as a CLI', () => {
    ...
    process.env.LOCALAPPDATA = localAppData;
    process.env.PATH = createTempDir('mittrcraft-empty-path-');
    process.env.SystemRoot = createTempDir('mittrcraft-empty-systemroot-');
    delete process.env.OPENCODE_BINARY;
    const { runtime } = createRuntime({}, {
      spawnSync: () => ({ status: 1, stdout: '', stderr: '' }),
    });
  ...
  it('skips Windows OpenCode desktop app entries returned by where.exe', () => {
    ...
    const localAppData = createTempDir('mittrcraft-localappdata-');
    const desktopBinary = path.join(localAppData, 'Programs', 'OpenCode', 'OpenCode.exe');
    const cliBinary = path.join(createTempDir('mittrcraft-cli-'), 'opencode.exe');
    fs.mkdirSync(path.dirname(desktopBinary), { recursive: true });
    fs.writeFileSync(desktopBinary, '');
  ...
  it('rejects WSL settings in strict mode', async () => {
    setPlatform('win32');
    const dir = createTempDir('mittrcraft-no-wsl-');
    process.env.PATH = dir;
    process.env.SystemRoot = dir;
    process.env.WSL_BINARY = path.join(dir, 'missing-wsl.exe');
    process.env.MITTRCRAFT_WSL_BINARY = path.join(dir, 'missing-mittrcraft-wsl.exe');
  ...
  it('does not auto-detect OpenCode from WSL fallback paths', () => {
    setPlatform('win32');
    const dir = createTempDir('mittrcraft-wsl-opencode-');
  ...
  it('launches Windows cmd shims through cmd call without embedded quotes', () => {
    ...
    const dir = createTempDir('mittrcraft-opencode-cmd-');
  ...
  it('resolves npm OpenCode cmd shims to the packaged Windows executable', () => {
    setPlatform('win32');
    const npmDir = createTempDir('mittrcraft-opencode-npm-');
