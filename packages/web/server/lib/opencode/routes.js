...
<title>${escapeHtml(title)} — Mittr Craft</title>
<style>
...
${desktopReturn ? `<a class="return" href="openchamber://focus/mcp-auth">Return to Mittr Craft</a>
<script>window.location.href = 'openchamber://focus/mcp-auth';</script>` : ''}
      ...
      if (!capability.supported) {
        return res.status(409).json({
          ...
          error: capability.reason === 'bundled'
            ? 'OpenCode is bundled with Mittr Craft Desktop and updates with the app.'
            : 'This OpenCode runtime cannot be upgraded by Mittr Craft.',
        });
      }
  ...
  // Mittr Craft UI session — the SPA route this path used to land on sits
  ...
  app.get('/mcp/oauth/callback', async (req, res) => {
    ...
    if (!context?.name) {
      return finish(400, {
        title: 'Authorization Failed',
        message: 'This authorization session has expired or is unknown to the running app. Return to Mittr Craft and click Authorize again.',
      });
    }
    ...
    try {
      ...
      return finish(200, {
        ...
        message: 'You can close this tab and return to Mittr Craft.',
      });
    } catch (error) {
      return finish(502, {
        title: 'Authorization Failed',
