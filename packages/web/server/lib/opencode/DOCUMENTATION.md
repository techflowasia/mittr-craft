...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/MittrCraft route wiring.
...
- `packages/web/server/lib/opencode/mittrcraft-routes.js`: MittrCraft update and models metadata route registration.
...
MittrCraft tool injection. Managed launch env strips AppImage `ARGV0` before
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside MittrCraft).
...
The pipeline binds the MittrCraft listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when MittrCraft binds port `0`.

## Public exports (mittrcraft-routes.js)
- `registerMittrCraftRoutes(app, dependencies)`: registers MittrCraft endpoints:
  - `GET /api/mittrcraft/update-check`
  - `POST /api/mittrcraft/update-install`
    - Foreground servers running under a systemd user unit queue installation in
      ...
      `MITTRCRAFT_SYSTEMD_UNIT` overrides the default `mittrcraft.service`.
  - `GET /api/mittrcraft/models-metadata`
  - `GET /api/zen/models`
...
- Rate limit config: `MITTRCRAFT_RATE_LIMIT_MAX_ATTEMPTS`, `MITTRCRAFT_RATE_LIMIT_NO_IP_MAX_ATTEMPTS` env vars.
