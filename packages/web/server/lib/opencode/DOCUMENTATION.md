...
- `packages/web/server/lib/opencode/bootstrap-runtime.js`: base app bootstrap runtime for status/auth/tts/notification/Mittr Craft route wiring.
...
- `packages/web/server/lib/opencode/openchamber-routes.js`: Mittr Craft update and models metadata route registration.
...
Mittr Craft tool injection. Managed launch env strips AppImage `ARGV0` before
...
- `registerSettingsUtilityRoutes(app, dependencies)`: registers small settings utility endpoints:
  ...
  - `POST /api/config/reload` — applies accumulated deferred OpenCode config changes. Managed OpenCode restarts and returns `requiresReload: true`. External OpenCode returns `requiresManualRestart: true` (changes are already on disk; the connected server must be restarted outside Mittr Craft).
...
The pipeline binds the Mittr Craft listener and publishes its active port
before starting managed OpenCode. The managed custom tool therefore receives
an authoritative loopback callback URL even when Mittr Craft binds port `0`.

## Public exports (openchamber-routes.js)
- `registerOpenChamberRoutes(app, dependencies)`: registers Mittr Craft endpoints:
