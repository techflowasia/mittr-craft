export const registerProjectIconRoutes = (app, dependencies) => {
  ...
  const projectIconsDirPath = path.join(mittrcraftDataDir, 'project-icons');
  ...
  const applyProjectIconSvgTheme = (svgMarkup, themeVariant, iconColor) => {
    ...
    const overrideStyle = `<style data-mittrcraft-theme-icon="1">:root{color:${color}!important;}</style>`;
