...
describe('static routes runtime', () => {
  ...
  it('returns API-only info JSON for JSON clients', async () => {
    ...
    expect(response.body).toEqual({
      ok: true,
      mode: 'api-only',
      message: 'MittrCraft is running in API-only mode',
    });
  });
  ...
  it('does not intercept API, auth, or health routes in API-only mode', async () => {
    ...
    expect(api.body).not.toEqual({ ok: true, mode: 'api-only', message: 'MittrCraft is running in API-only mode' });
    expect(auth.body).not.toEqual({ ok: true, mode: 'api-only', message: 'MittrCraft is running in API-only mode' });
    expect(health.body).not.toEqual({ ok: true, mode: 'api-only', message: 'MittrCraft is running in API-only mode' });
  });
});
