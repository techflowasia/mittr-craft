...
export const createScheduledTaskService = (dependencies) => {
  ...
  const findProjectByID = async (projectID) => {
    ...
    if (!normalized) throw new MittrCraftControlError('projectId is required', 400);
    const projects = await listProjects();
    const project = projects.find((entry) => entry.id === normalized) || null;
    if (!project) throw new MittrCraftControlError('Project not found', 404);
    return project;
  };
  ...
  const resolveProjectID = async ({ projectId, directory } = {}) => {
    ...
    if (requestedProjectID && requestedDirectory) {
      throw new MittrCraftControlError('Provide only one of projectId or directory', 400);
    }
    if (requestedProjectID) {
      await findProjectByID(requestedProjectID);
      return requestedProjectID;
    }
    if (!requestedDirectory) throw new MittrCraftControlError('projectId or directory is required', 400);
    ...
    if (!project) throw new MittrCraftControlError(`Project not found for directory: ${resolvedDirectory}`, 404);
    return project.id;
  };
  ...
  const findLoopTask = async (projectID, taskID) => {
    ...
    if (!normalizedTaskID) throw new MittrCraftControlError('taskId is required', 400);
    const tasks = await scheduledTasksRuntime.syncProject(projectID);
    const task = tasks.find((entry) => entry?.id === normalizedTaskID) || null;
    if (!task) throw new MittrCraftControlError('Task not found', 404);
    if (!task.loopFile) throw new MittrCraftControlError('Task is not managed by a loop file', 400);
    if (!fs.existsSync(task.loopFile)) throw new MittrCraftControlError('Loop file not found', 404);
    return task;
  };

  const setLoopEnabled = async (projectID, taskID, enabled) => {
    if (typeof enabled !== 'boolean') {
      throw new MittrCraftControlError('enabled must be a boolean', 400);
    }
    const task = await findLoopTask(projectID, taskID);
    try {
      if (!setLoopFileEnabled(task.loopFile, enabled)) {
        throw new MittrCraftControlError('Loop file must be valid before changing its enabled state', 400);
      }
    } catch (error) {
      if (error instanceof MittrCraftControlError) throw error;
      const message = error instanceof Error ? error.message : 'Failed to update loop file';
      throw new MittrCraftControlError(message, 500);
    }
    const tasks = await scheduledTasksRuntime.syncProject(projectID);
  ...
  const upsert = async (projectID, taskInput) => {
    ...
    if (!taskInput || typeof taskInput !== 'object') {
      throw new MittrCraftControlError('task payload is required', 400);
    }
    let upserted;
    try {
      ...
      throw new MittrCraftControlError(message, invalid ? 400 : 500);
    }
  ...
  const remove = async (projectID, taskID) => {
    ...
    if (!result.deleted) throw new MittrCraftControlError('Task not found', 404);
    await scheduledTasksRuntime.syncProject(projectID);
  ...
  const run = async (projectID, taskID) => {
    ...
    if (result.running || result.queued) {
      throw new MittrCraftControlError(result.error || 'Task already running', 409);
    }
    if (result.skipped) throw new MittrCraftControlError('Task not found or disabled', 404);
    if (!result.ok) {
      throw new MittrCraftControlError(result.error || 'Task run failed', 500, { task: result.task });
    }
    return {
      task: result.task,
