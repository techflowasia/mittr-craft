...
Server-owned scheduled task runtime and routes for MittrCraft-only automation.
...
- This module is MittrCraft feature logic; it is intentionally separate from OpenCode proxy/runtime internals.

## Cross-instance occurrence claiming

Multiple MittrCraft server processes can share the same on-disk project config
...
- `packages/web/server/lib/scheduled-tasks/runtime.js`
  ...
  - Emits MittrCraft task-run events
...
- `packages/web/server/lib/scheduled-tasks/routes.js`
  ...
  - Manual run endpoint
  - MittrCraft events SSE stream endpoint
...
- Registers:
  ...
  - `GET /api/mittrcraft/scheduled-tasks/status`
  - `GET /api/mittrcraft/events`
