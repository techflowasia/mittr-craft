...
## Purpose
This module contains the MittrCraft message-stream WebSocket protocol and runtime bridge. It keeps the browser-facing WebSocket transport separate from the upstream OpenCode SSE transport.
...
- MittrCraft still fetches OpenCode upstream event streams over SSE.
...
- Global synthetic events such as `mittrcraft:session-status`, `mittrcraft:session-activity`, `mittrcraft:notification`, and `mittrcraft:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
