...
// Routes are registered with the other Mittr Craft feature routes, before the
...
export const createRelayService = ({
  ...
  const standbyStatus = (holderPid) => ({
    state: 'standby',
    lastError: `relay host is owned by another local Mittr Craft process (pid ${holderPid})`,
    connectedClients: 0,
  });
