...
export const isUnsafeUnauthenticatedLanAllowed = (env = process.env) =>
  env?.MITTRCRAFT_ALLOW_UNAUTHENTICATED_LAN === 'true';

export const getUnauthenticatedLanErrorMessage = (host) =>
  `MittrCraft refuses to bind to ${host || 'a network-exposed host'} without UI authentication. `
  + 'Set --ui-password or MITTRCRAFT_UI_PASSWORD before exposing it over LAN, '
  + 'or set MITTRCRAFT_ALLOW_UNAUTHENTICATED_LAN=true to accept the risk.';
