# Managed MittrCraft Agent Tool

## Purpose

This module exposes MittrCraft to agents as typed OpenCode custom tools. There
are two, because controlling sessions and driving a page are separate intents
the user can want independently:

- `mittrcraft` — projects, sessions, worktrees, and scheduled tasks. Enabled
  while the persisted `agentControlToolEnabled` setting is not `false`.
- `mittrcraft_web` — looking at and interacting with the page in MittrCraft's
...
when MittrCraft launches and owns the OpenCode process, and not at all when
...
1. The MittrCraft HTTP listener binds and publishes its authoritative port.
2. `prepareManagedOpenCodeEnv()` materializes the plugin under
   `<mittrcraft-data-dir>/agent-tool/` and appends its `file://` URL to
...
4. The plugin calls `POST /api/mittrcraft/agent-tool` with its typed input and
   OpenCode's authoritative session directory.
5. The route delegates the fixed action allowlist directly to the shared
   MittrCraft control service. The CLI uses the same service through its
...
- The tool exposes only agent-relevant actions
  (`MITTRCRAFT_AGENT_TOOL_ACTIONS`): `schedule.status` stays CLI-only because
...
- External OpenCode selected with `OPENCODE_HOST` or skip-start: not injected,
  because MittrCraft does not control that process environment.
