...
plugins — this module replicates that mechanism as a MittrCraft runtime API.
...
- `resolve.js` — model selection, mirroring OpenCode's `getSmallModel` chain:
  0. MittrCraft's own settings override (Settings → Sessions → Small Model):
...
- `catalog.js` — models.dev catalog via the shared in-process cache
  ...
  `/api/mittrcraft/models-metadata`).
