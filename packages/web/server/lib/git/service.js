...
const createGit = async (directory, { allowUnsafeSshCommand = false } = {}) => {
  ...
  // makes simple-git use process.cwd(), which breaks when the Mittr Craft
...
// Mittr Craft places managed worktrees under a deep data-dir path
...
const formatWorktreePopulateError = (message) => {
  ...
  return [
    ...
    'Mittr Craft enables Git `core.longpaths` for worktree population; if this still fails on Windows, enable OS long paths (LongPathsEnabled) or open the repository from a shorter absolute path.',
  ].join('\n');
};
...
export const ensureWorktreeLongpaths = async (directory) => {
  ...
  // subsequent Mittr Craft and CLI git operations in this repo also get long
...
// whose directory no longer exists when reading them back. Mittr Craft used to
...
export async function stashPush(directory, options = {}) {
  ...
  const message = typeof options.message === 'string' && options.message.trim()
    ...
    : `Mittr Craft stash ${new Date().toISOString()}`;
