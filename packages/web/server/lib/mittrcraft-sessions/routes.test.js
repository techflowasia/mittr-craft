...
globalThis.__mittrcraftCreateWorktreeMock = createWorktreeMock;
globalThis.__mittrcraftGetWorktreeBootstrapStatusMock = getWorktreeBootstrapStatusMock;

let registerMittrCraftSessionRoutes;
...
vi.mock('../git/index.js', () => ({
  createWorktree: (...args) => globalThis.__mittrcraftCreateWorktreeMock(...args),
  getWorktreeBootstrapStatus: (...args) => globalThis.__mittrcraftGetWorktreeBootstrapStatusMock(...args),
}));
...
describe('mittrcraft session routes', () => {
  beforeAll(async () => {
    ({ registerMittrCraftSessionRoutes } = await import('./routes.js'));
  });
  ...
  it('creates a worktree before creating a session', async () => {
    ...
    try {
      ...
      const response = await request(app)
        ...
        .send({
          directory: '/repo/app',
          worktree: { name: 'side-task', branchName: 'mittrcraft/side-task', startRef: 'main' },
          setUpstream: false,
          prompt: 'Run this',
          model: 'openai/gpt-5.5',
        })
  ...
  it('sends a goal prompt to an existing session after creating goal metadata', async () => {
    ...
    try {
      ...
      const { app } = createApp({ createSessionGoal });
      const response = await request(app)
        .post('/api/mittrcraft/sessions/ses_source/send')
        .send({
          directory: '/repo/app',
  ...
  it('forks from a message, dispatches the prompt, and emits the new session', async () => {
    ...
    try {
      const { app } = createApp({ emitSessionCreatedEvent });
      const response = await request(app)
        .post('/api/mittrcraft/sessions/ses_source/fork')
        .send({
          directory: '/repo/app',
