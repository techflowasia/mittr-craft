...
import { MittrCraftControlError, asControlError } from '../mittrcraft-control/error.js';
...
const waitForWorktreeBootstrapReady = async ({ directory }) => {
  ...
  for (;;) {
    ...
    if (status?.status === 'failed') {
      throw new MittrCraftControlError(`Worktree bootstrap failed: ${status.error || 'unknown error'}`, 500);
    }
    const phase = status?.phase;
    if (status?.status === 'ready' || phase === 'git-ready' || phase === 'setup-ready') return;
    if (Date.now() >= deadline) {
      throw new MittrCraftControlError('Timed out waiting for the worktree bootstrap', 500);
    }
...
export const createMittrCraftSessionService = (dependencies) => {
  const {
  ...
  const validateRequestedSelection = async ({ directory, requestedModel, requestedAgent, requestedVariant }) => {
    ...
    if (requestedAgent && agents.length > 0) {
      ...
      if (!agent) {
        throw new MittrCraftControlError(`Unknown agent '${requestedAgent}' for ${directory}`, 400);
      }
      if (!isPrimaryAgentMode(agent.mode)) {
        throw new MittrCraftControlError(`Agent '${requestedAgent}' is a subagent and cannot receive a prompt directly`, 400);
      }
    }
  ...
  const dispatchPrompt = async ({
    ...
    if (goalInput.enabled) {
      ...
      await (createSessionGoalOverride || createSessionGoal)({
        ...
        onWarning: (message, error) => console.warn(`[MittrCraftSessions] ${message}:`, error?.message || error),
      });
    }
  ...
  const create = async (payload = {}) => {
    ...
    if (!goalInput.ok) {
      throw new MittrCraftControlError(goalInput.error, 400);
    }
    ...
    if (!resolvedDirectory.ok) {
      throw new MittrCraftControlError(resolvedDirectory.error, resolvedDirectory.status || 400);
    }
    ...
    if (payload?.worktree && !worktreeInput) {
      throw new MittrCraftControlError('worktree.name is required when worktree is provided', 400);
    }
  ...
  const runExisting = async (action, sourceSessionId, payload = {}) => {
    ...
    const prompt = asNonEmptyString(payload.prompt);
    if (!sourceSessionID) throw new MittrCraftControlError('sessionId is required', 400);
    if (!prompt) throw new MittrCraftControlError('prompt is required', 400);
    const goalInput = resolveGoalInput(payload, prompt);
    if (!goalInput.ok) throw new MittrCraftControlError(goalInput.error, 400);
...
export const registerMittrCraftSessionRoutes = (app, dependencies) => {
  const service = dependencies.sessionService || createMittrCraftSessionService(dependencies);

  app.post('/api/mittrcraft/sessions', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      return res.json(await service.create(req.body && typeof req.body === 'object' ? req.body : {}));
    } catch (error) {
      console.error('[MittrCraftSessions] failed to create session:', error);
      return sendServiceError(res, error, 'Failed to create session');
    }
  });

  app.post(
    '/api/mittrcraft/sessions/:sessionId/send',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.send(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[MittrCraftSessions] failed to send session:', error);
        return sendServiceError(res, error, 'Failed to send session');
      }
    },
  );
  app.post(
    '/api/mittrcraft/sessions/:sessionId/fork',
    express.json({ limit: '1mb' }),
    async (req, res) => {
      try {
        return res.json(await service.fork(req.params.sessionId, req.body));
      } catch (error) {
        console.error('[MittrCraftSessions] failed to fork session:', error);
