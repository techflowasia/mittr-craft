...
export const createOpenChamberControlService = (dependencies) => {
  ...
  const wait = (duration, signal) => {
    ...
    if (signal.aborted) return Promise.reject(new OpenChamberControlError('Mittr Craft action was cancelled', 499));
    return new Promise((resolve, reject) => {
      const onAbort = () => {
        signal.removeEventListener('abort', onAbort);
        reject(new OpenChamberControlError('Mittr Craft action was cancelled', 499));
      };
  ...
  const waitForIdle = async ({ client, sessionID, directory, timeoutMs, requireActivity, baselineMessageID, startedAt, signal }) => {
    ...
    while (true) {
      if (signal?.aborted) throw new OpenChamberControlError('Mittr Craft action was cancelled', 499);
  ...
  const execute = async (action, input = {}, contextDirectory, options = {}) => {
    try {
      if (!CONTROL_ACTIONS.has(action)) {
        throw new OpenChamberControlError(`Unsupported Mittr Craft action: ${action || 'missing'}`, 400);
      }
      ...
      throw asControlError(error, `Failed to execute ${action || 'Mittr Craft action'}`);
    }
  };
