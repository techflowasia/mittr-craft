...
const pwaDevEnabled = process.env.MITTRCRAFT_DISABLE_PWA_DEV !== '1';
...
const themeJsonHmrPlugin = () => ({
  name: 'mittrcraft-theme-json-hmr',
  handleHotUpdate({ file, server }: { file: string; server: { ws: { send: (payload: unknown) => void } } }) {
    ...
    try {
      server.ws.send({
        type: 'custom',
        event: 'mittrcraft:theme-updated',
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: '@mittrcraft/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.MITTRCRAFT_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.MITTRCRAFT_PORT || 3001}`,
