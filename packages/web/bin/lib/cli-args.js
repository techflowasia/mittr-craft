...
function showHelp() {
  console.log(`
 Mittr Craft - Web interface for the OpenCode AI coding agent
...
ENVIRONMENT:
  ...
  OPENCHAMBER_DATA_DIR         Override Mittr Craft data directory
...
EXAMPLES:
  ...
  openchamber startup enable     # Start Mittr Craft at user login
...
function showControlHelp() {
  console.log(`
 Mittr Craft Control Commands

USAGE:
  openchamber <COMMAND> [OPTIONS]

COMMANDS:
  status                         Show running Mittr Craft runtimes
...
COMMON OPTIONS:
  ...
  -p, --port <port>              Target a specific Mittr Craft runtime
...
DESCRIPTION:
  ...
  Mittr Craft app. If no server is running on the selected port, it starts one.
...
COMMON OPTIONS:
  -p, --port              Target Mittr Craft instance port
...
BEHAVIOR NOTES:
  - One active tunnel per Mittr Craft instance.
