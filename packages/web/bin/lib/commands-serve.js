...
async function serveCommand(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'MittrCraft serve' });
    }
    ...
    if (options.foreground) {
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting MittrCraft on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    child.unref();
    serveSpin?.start(`Starting MittrCraft on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`MittrCraft daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`MittrCraft daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start MittrCraft');
