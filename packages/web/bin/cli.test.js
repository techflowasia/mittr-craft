...
describe('cli API target resolution', () => {
  ...
  it('fails when multiple non-default instances are running', async () => {
    await expect(resolveTargetPort({}, {
      ...
      isServerHealthReady: async () => false,
    })).rejects.toThrow('Multiple Mittr Craft instances are running');
  });
});
...
describe('isOpenchamberCmdline', () => {
  it('accepts Mittr Craft CLI and daemon cmdlines', () => {
...
describe('isOpenchamberProcessRunning', () => {
  ...
  it.skipIf(process.platform !== 'linux' && process.platform !== 'darwin')(
    'returns false for a live non-Mittr Craft PID',
    async () => {
...
describe('lifecycle instance discovery', () => {
  ...
  it('uses the live system-info pid instead of a stale Mittr Craft-looking pid-file pid', async () => {
    await withTempOpenChamberDataDir(async () => {
      const port = 45127;
  ...
  it('discovers an explicit live Mittr Craft port without a pid-file registry entry', async () => {
    await withTempOpenChamberDataDir(async () => {
      const port = 45125;
      const instances = await discoverLifecycleInstances(
...
describe('lifecycle commands with unmanaged explicit ports', () => {
  it('serve refuses to start on a live Mittr Craft port without requiring pid files', async () => {
    await withTempOpenChamberDataDir(async () => {
