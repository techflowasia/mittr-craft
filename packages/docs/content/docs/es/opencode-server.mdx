...
description: Conecta Mittr Craft a un servidor de OpenCode local o remoto.
---

# OpenCode Server

Mittr Craft se ejecuta sobre un servidor de OpenCode. De forma predeterminada inicia uno por ti, así que no tienes que hacer nada. Solo necesitas esta página si quieres apuntar Mittr Craft a un servidor que ya ejecutas, o gestionar el que él inicia.

## Cómo encuentra Mittr Craft un servidor

Cuando Mittr Craft arranca, busca un servidor en este orden:
...
Define esto antes de iniciar Mittr Craft:
...
- `OPENCODE_SKIP_START=true` — le indica a Mittr Craft que no inicie su propio servidor.

Si solo necesitas cambiar el puerto, define `OPENCODE_PORT` en lugar de `OPENCODE_HOST`.

Si a `OPENCODE_HOST` le falta el puerto o tiene una ruta, Mittr Craft lo ignora y recurre a iniciar su propio servidor. Observa los registros de arranque por si aparece una advertencia `[config]` cuando una conexión que esperabas no ocurrió.
...
## Iniciar Mittr Craft al iniciar sesión

Usa `startup enable` para instalar un servicio nativo de usuario. Mittr Craft usa `launchd` en macOS, `systemd --user` en Linux y Task Scheduler en Windows.
...
Para crear un enlace de conexión para otra app de Mittr Craft, usa:
...
Mientras el servidor se inicia o reinicia, Mittr Craft muestra un estado "OpenCode is restarting" y pausa las solicitudes hasta que esté listo. Esto es normal justo después de iniciar o reiniciar. Si nunca desaparece, consulta [Conexión de OpenCode](/es/troubleshooting/opencode-connection/).
