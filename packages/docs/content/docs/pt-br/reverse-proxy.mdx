---
title: Proxy reverso
description: Configure o Mittr Craft corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o Mittr Craft atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o Mittr Craft funciona diretamente.
...
- Desative o gzip no proxy se o Mittr Craft já comprime as respostas.
...
- Mittr Craft acessível diretamente na LAN
...
- O Mittr Craft comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo Mittr Craft, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
