---
title: Tunele
description: Bezpiecznie udostępnij Mittr Craft do dostępu zdalnego i mobilnego.
---

# Tunele

Tunel to publiczny link do Twojego Mittr Craft, dzięki któremu możesz dotrzeć do niego ze zwykłej przeglądarki w innej sieci. Użyj `openchamber tunnel`, aby utworzyć go dla działającej instancji.
...
Mittr Craft uruchamia CLI dostawcy tunelu na Twoim komputerze. Najpierw zainstaluj dostawcę, którego chcesz użyć:
...
Gdy tunel działa, `status` pokazuje publiczny adres URL. Otwórz go lub zeskanuj kod QR, aby dotrzeć do Mittr Craft z dowolnego miejsca.

Domyślnie Mittr Craft wypisuje kod QR w interaktywnych sesjach TTY. Użyj `--qr`, aby wymusić wyświetlenie kodu QR, lub `--no-qr`, aby je wyłączyć.
...
- jeden aktywny tunel na instancję Mittr Craft (port)
