---
title: Reverse proxy
description: Skonfiguruj Mittr Craft poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Mittr Craft za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Mittr Craft działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Mittr Craft już kompresuje odpowiedzi.
...
- Mittr Craft dostępny bezpośrednio w LAN
...
- Mittr Craft kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Mittr Craft, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
