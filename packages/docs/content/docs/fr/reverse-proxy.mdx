---
title: Reverse proxy
description: Configurez correctement MittrCraft derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez MittrCraft derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord qu’MittrCraft fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si MittrCraft compresse déjà les réponses.
...
- MittrCraft joignable directement sur le LAN
...
- MittrCraft compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par MittrCraft, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
