---
title: Tunnels
description: Exposez MittrCraft en sécurité pour l’accès distant et mobile.
---

# Tunnels

Un tunnel est un lien public vers votre MittrCraft, pour y accéder depuis un navigateur ordinaire sur un autre réseau. Utilisez `mittrcraft tunnel` pour en créer un pour une instance en cours d’exécution.
...
MittrCraft démarre la CLI du fournisseur de tunnel sur votre machine. Installez d’abord le fournisseur que vous voulez utiliser :
...
1. Démarrez MittrCraft :

```bash
mittrcraft
```

Si vous sautez cette étape, `mittrcraft tunnel start` peut démarrer automatiquement un serveur CLI. Lors de l’auto-démarrage, vous pouvez passer des options serveur comme `--port`, `--host`, `--lan`, `--ui-password` et `--api-only`.
...
Quand le tunnel est actif, `status` affiche une URL publique. Ouvrez-la, ou scannez le QR code, pour joindre MittrCraft depuis n’importe où.

Par défaut, MittrCraft affiche un QR code dans les sessions TTY interactives. Utilisez `--qr` pour forcer la sortie QR, ou `--no-qr` pour la désactiver.
...
- un tunnel actif par instance MittrCraft (port)
...
- [PWA et mobile](/mobile/) — joindre MittrCraft depuis votre téléphone
