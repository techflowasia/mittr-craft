...
description: Konfiguriere die Mittr Craft- und OpenCode-Integration mit Umgebungsvariablen.
---

# Umgebungsvariablen

Mittr Craft liest diese Umgebungsvariablen beim Start. Für Startdienste speichert `openchamber startup enable` die aktuelle Umgebung standardmäßig als Schnappschuss, also führe den Befehl nach Änderungen an den Variablen, die der Dienst verwenden soll, erneut aus.

## Mittr Craft-Server

### `OPENCHAMBER_HOST`

Bind-Adresse für den Mittr Craft-Webserver. Verwende `0.0.0.0`, um Zugriff von anderen Maschinen zu erlauben.
...
Startet Mittr Craft im Headless-Modus, wenn auf `true` oder `1` gesetzt. API-Routen bleiben für Desktop- und Mobile-Clients verfügbar, aber die Browser-Oberfläche wird nicht ausgeliefert.

### `OPENCHAMBER_DATA_DIR`

Überschreibt das Mittr Craft-Datenverzeichnis. Standard ist `~/.config/openchamber`.
...
Verbindet Mittr Craft mit einem vorhandenen OpenCode-Server. Der Wert muss ein `http`- oder `https`-Origin mit explizitem Port und ohne Pfad, Query oder Hash sein. `OPENCODE_HOST` hat Vorrang vor `OPENCODE_PORT`.
...
Verhindert, dass Mittr Craft einen eigenen OpenCode-Server startet, wenn auf `true` gesetzt.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Bind-Hostname für den von Mittr Craft verwalteten OpenCode-Server. Standard ist `127.0.0.1`.

### `OPENCODE_BINARY`

Pfad zur ausführbaren `opencode`-Datei, die Mittr Craft ausführen soll.
...
Mittr Craft-spezifischer Alias zur Auswahl der WSL-Distribution. `OPENCODE_WSL_DISTRO` hat Vorrang, wenn beide gesetzt sind.
...
Shell-Executable, die Mittr Craft für Terminal-Sessions verwendet.

### `OPENCHAMBER_GIT_BINARY`

Git-Executable, die Mittr Craft-Git-Funktionen verwenden.

### `GIT_BINARY`

Alternative Git-Executable-Überschreibung. Bevorzuge `OPENCHAMBER_GIT_BINARY` für Mittr Craft-spezifische Konfiguration.
...
API-Schlüssel, den Mittr Craft-Voice-Funktionen für OpenAI-kompatible Dienste verwenden.
...
ngrok-Auth-Token, den Mittr Craft-Tunnel-Befehle verwenden. Du kannst ngrok auch mit `ngrok config add-authtoken <token>` konfigurieren.

## Laufzeit-Helfer

### `BUN_BINARY`

Bun-Executable, die Mittr Craft beim Starten von Daemon-Prozessen verwenden soll.

### `BUN_INSTALL`

Installationswurzel von Bun. Mittr Craft verwendet sie, um `bin/bun` für Daemon-Start und Updates zu finden.
