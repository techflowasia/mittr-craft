---
title: Remote Access
description: Fix tunnels, remote instances, and reaching Mittr Craft from another device.
---

# Remote Access

When you can't reach Mittr Craft from your phone or another machine, the fix depends on how you're connecting.
...
- for **Anywhere** pairing, check **Settings → Remote Instances → Mittr Craft Relay** on the server: it should say **Connected**; if not, disable and re-enable it
...
When a [remote instance](/remote-instances/) stalls, Mittr Craft names the step that failed:

- **auth** — your SSH or UI password was rejected; re-enter it
- **install / start** — Mittr Craft couldn't set up or start the server on the remote machine; check that machine's requirements
- **forwarding** — the connection is up but the port isn't reaching you; try a different local port

## Behind your own server

If you put Mittr Craft behind a reverse proxy and it loads oddly or won't connect, see [Reverse Proxy](/reverse-proxy/).
