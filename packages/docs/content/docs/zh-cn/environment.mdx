---
title: 环境变量
description: 使用环境变量配置 Mittr Craft 和 OpenCode 集成。
---

# 环境变量

Mittr Craft 会在启动时读取这些变量。对于开机启动服务，`openchamber startup enable` 默认会保存当前环境的 snapshot，所以如果要让服务使用新的变量，请重新运行它。

## Mittr Craft 服务器

### `OPENCHAMBER_HOST`

Mittr Craft web 服务器监听的地址。使用 `0.0.0.0` 可允许其他机器访问。
...
设置为 `true` 或 `1` 时，以 headless 模式启动 Mittr Craft。桌面和移动客户端仍可使用 API 路由，但不会提供浏览器 UI。

### `OPENCHAMBER_DATA_DIR`

覆盖 Mittr Craft 数据目录。默认是 `~/.config/openchamber`。
...
将 Mittr Craft 连接到已有的 OpenCode server。值必须是带明确端口、没有 path、query 或 hash 的 `http` 或 `https` origin。`OPENCODE_HOST` 优先于 `OPENCODE_PORT`。
...
设置为 `true` 时阻止 Mittr Craft 启动自己的 OpenCode server。

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Mittr Craft 管理的 OpenCode server 绑定 hostname。默认是 `127.0.0.1`。

### `OPENCODE_BINARY`

Mittr Craft 应该运行的 `opencode` executable 路径。
...
Mittr Craft 专用的 WSL distro alias。如果两个变量都设置，`OPENCODE_WSL_DISTRO` 优先。
...
Mittr Craft terminal sessions 使用的 shell executable。

### `OPENCHAMBER_GIT_BINARY`

Mittr Craft Git 功能使用的 Git executable。

### `GIT_BINARY`

Git executable 的备用 override。Mittr Craft 专用配置建议使用 `OPENCHAMBER_GIT_BINARY`。
...
Mittr Craft 语音功能调用 OpenAI-compatible services 时使用的 API key。
...
Mittr Craft tunnel commands 使用的 ngrok auth token。也可以用 `ngrok config add-authtoken <token>` 配置。

## Runtime helpers

### `BUN_BINARY`

Mittr Craft 启动 daemon processes 时使用的 Bun executable。

### `BUN_INSTALL`

Bun 安装根目录。Mittr Craft 使用它查找 `bin/bun` 来启动 daemon 和执行更新。
