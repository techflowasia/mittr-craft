---
title: Tunnels
description: Expose Mittr Craft safely for remote and mobile access.
---

# Tunnels

A tunnel is a public link to your Mittr Craft, so you can reach it from an ordinary browser on another network. Use `openchamber tunnel` to create one for a running instance.
...
Mittr Craft starts the tunnel provider CLI on your machine. Install the provider you want to use first:
...
When the tunnel is up, `status` shows a public URL. Open it, or scan the QR code, to reach Mittr Craft from anywhere.

By default, Mittr Craft prints a QR code in interactive TTY sessions. Use `--qr` to force QR output, or `--no-qr` to disable it.
...
- one active tunnel per Mittr Craft instance (port)
