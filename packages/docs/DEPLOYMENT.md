...
Rendering and hosting still happen in `mittrcraft-website` (`apps/docs`).
...
- creates `mittrcraft-docs-source-<sha>.tar.gz`
...
to `mittrcraft-website` so it re-syncs and redeploys the docs. This fires on
...
- `MITTRCRAFT_WEBSITE_REPO_TOKEN` — a token with `contents: write` (classic
  `repo` scope, or fine-grained with Contents: read & write) on
  `techflowasia/mittr-craft-website`. **Without it the dispatch step is skipped**
...
- `source_repo`
- `source_ref` — the ref the website checks out from `mittrcraft` (`main` on a
  push, the tag on a release)
- `archive_name`

`mittrcraft-website`'s `deploy-docs.yml` listens for this event
(`repository_dispatch: types: [docs_source_updated]`), checks out
`mittrcraft` at `source_ref`, runs `docs:sync`, builds `apps/docs`, and deploys
to Cloudflare Pages. That repo needs its own secrets: `MITTRCRAFT_REPO_TOKEN`
