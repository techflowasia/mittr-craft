...
This package is docs content source-of-truth for MittrCraft.
...
- Lead with the task, not background. The first line should say what the page is
  for ("Use `mittrcraft tunnel` to expose a running MittrCraft instance.").
...
- A basic page should fit in a screen or two. Long, dense reference pages (like
  ...
  this page if you run MittrCraft behind...").
...
```mdx
3. Run `mittrcraft --ui-password be-creative-here`.
4. Open the printed URL (usually `http://localhost:3000`).

You should land on the MittrCraft session list. If you see it, the server is
running.
```
...
- Explain a term the first time it appears, in parentheses, in everyday words:
  - good: start a tunnel (a public link to your local MittrCraft)
...
2. Add frontmatter at top:
   ...
   ---
   title: Remote Access
   description: Access MittrCraft from outside your local network.
   ---
   ```
...
The docs are translated into the same languages the MittrCraft app ships in.
...
`mittrcraft-website/apps/docs/astro.config.mjs` (`locales`). If a language is
...
- brand and product nouns: MittrCraft, OpenCode, VS Code, PWA, GitHub, Discord,
  macOS, SSH
...
## Sync into mittrcraft-website

`mittrcraft-website` renders/deploys docs via Starlight in `apps/docs`.

After docs content updates here:

1. copy `packages/docs/content/docs/*` -> `mittrcraft-website/apps/docs/src/content/docs/*`
   (this is recursive, so locale folders like `uk/` and `zh-cn/` carry over with
   no extra steps)
2. map `packages/docs/sidebar.config.json` into `mittrcraft-website/apps/docs/astro.config.mjs` sidebar
