...
describe('ElectronSshManager', () => {
  ...
  test('runs each Windows port forward as an independent hidden SSH process', async () => {
    ...
    manager.sshAuth.set(parsed, {
      askpassPath: 'C:\\Mittr Craft\\askpass.cmd',
    ...
    for (const call of calls) {
      ...
      expect(call.options.env.SSH_ASKPASS).toBe('C:\\Mittr Craft\\askpass.cmd');
  ...
  test('stores a client token for forwarded Mittr Craft hosts when UI password is configured', async () => {
