...
Electron desktop runtime for MittrCraft on macOS, Windows, and Linux.

This package owns the native shell: windows, menus, deep links, native notifications, auto-updates, host switching, SSH connections, tunnel helpers, and packaged desktop builds. The web UI and MittrCraft server logic still live in `packages/web` and shared React UI lives in `packages/ui`.

## How It Runs

Desktop starts the MittrCraft web server in the same Electron main process. There is no separate sidecar subprocess for the MittrCraft server.

`main.mjs` imports `@mittrcraft/web/server/index.js` and calls `startWebUiServer()`. The Electron window then loads the UI from the local server in development, or from packaged `resources/web-dist` assets in packaged builds.
...
The preload bridge exposes desktop-only APIs to the web UI through `window.__MITTRCRAFT_DESKTOP__`. Privileged commands are checked in `main.mjs`, not only in the UI.
...
Linux AppImages must be built natively. Set `MITTRCRAFT_TARGET_ARCH=x64` or `MITTRCRAFT_TARGET_ARCH=arm64` when packaging; the build rejects a target that does not match the Linux host. The same target selects the bundled OpenCode CLI, native Electron rebuild, and Electron Builder architecture. Linux identity is stable across architectures: executable `mittrcraft`, desktop file `mittrcraft.desktop`, icon `mittrcraft`, and `StartupWMClass=mittrcraft`.
...
2. Environment overrides: `OPENCODE_BINARY`, `OPENCODE_PATH`, `MITTRCRAFT_OPENCODE_PATH`, or `MITTRCRAFT_OPENCODE_BIN`.
...
| Variable | Use |
|----------|-----|
| `MITTRCRAFT_ELECTRON_DEV=1` | Marks the runtime as desktop development mode |
| `MITTRCRAFT_ELECTRON_USE_BUNDLED_UI=1` | Uses staged web assets instead of the HMR dev server |
| `MITTRCRAFT_SKIP_LOCAL_SERVER=1` | Skips the in-process local MittrCraft server and uses the configured default remote instance; Desktop imports this from the user's login-shell environment, and packaged/bundled UI remains available for connection recovery |
...
| `MITTRCRAFT_SKIP_API_COMPRESSION=true` | Defaulted by Desktop to reduce local CPU overhead |
| `MITTRCRAFT_STARTUP_PERF=1` | Enables privacy-safe startup phase timings in Desktop/server logs; disabled by default |
...
- The browser panel's own session (`persist:mittrcraft-browser`): its storage is
...
2. Add the real command handling in `main.mjs` under `mittrcraft:invoke`.
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs are written through the platform log path for the `MittrCraft` app name.

Development builds use a separate user data directory named `MittrCraft Dev`, so dev state does not overwrite normal packaged app state.
...
- Keep `@mittrcraft/web`, `bun-pty`, `node-pty`, and native modules external in `bundle-main.mjs`; bundling them can break Electron startup.
