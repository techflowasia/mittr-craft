...
//
// Surfaces a glanceable, always-visible view of Mittr Craft's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'Mittr Craft — no active sessions';
  const bits = [];
  ...
  return `Mittr Craft — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show Mittr Craft', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit Mittr Craft', click: () => onAction({ type: 'quit' }) },
      );
    }
