#!/usr/bin/env bash
# MittrCraft Install Script
# Usage: curl -fsSL https://raw.githubusercontent.com/btriapitsyn/mittrcraft/main/scripts/install.sh | bash

set -euo pipefail

PACKAGE_NAME="@mittrcraft/web"
BIN_NAME="mittrcraft"
MIN_NODE_VERSION=22
...
main() {
  ...
  # If MittrCraft is already installed, hand off to its own updater instead
  # of guessing a package manager. `mittrcraft update` detects which manager
  ...
  if command_exists "$BIN_NAME"; then
    info "MittrCraft is already installed — updating via 'mittrcraft update'..."
    echo ""
    if mittrcraft update; then
      echo ""
      success "MittrCraft is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "MittrCraft installed successfully!"
    echo ""
    ...
    if command_exists "$BIN_NAME"; then
      echo "  Get started:"
      echo "    mittrcraft              # Start server on port 3000"
      echo "    mittrcraft --help       # Show all options"
    else
      ...
      if [ -n "$bin_dir" ]; then
        ...
        echo "  then restart your terminal and run: mittrcraft"
      fi
    fi
    echo ""
    echo "  Prerequisites:"
