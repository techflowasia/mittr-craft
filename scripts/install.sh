...
main() {
  ...
  # If Mittr Craft is already installed, hand off to its own updater instead
  ...
  if command_exists "$BIN_NAME"; then
    info "Mittr Craft is already installed — updating via 'openchamber update'..."
    echo ""
    if openchamber update; then
      echo ""
      success "Mittr Craft is up to date!"
      exit 0
    fi
    echo ""
    error "Update failed."
...
EOF
    printf '%b\n' "$NC"
    success "Mittr Craft installed successfully!"
    echo ""
