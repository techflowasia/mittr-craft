...
# alias; MITTRCRAFT_UI_PASSWORD is the canonical runtime variable.
if [ -z "${MITTRCRAFT_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  MITTRCRAFT_UI_PASSWORD="$UI_PASSWORD"
  export MITTRCRAFT_UI_PASSWORD
fi

if [ -n "${MITTRCRAFT_UI_PASSWORD:-}" ]; then
...
MITTRCRAFT_HOST="${MITTRCRAFT_HOST:-0.0.0.0}"
export MITTRCRAFT_HOST
...
if [ -n "${MITTRCRAFT_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$MITTRCRAFT_UI_PASSWORD"
fi
"$@"
